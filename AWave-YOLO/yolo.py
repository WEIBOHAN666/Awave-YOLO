...
'''
A must-read annotation for training your own data set!
'''
class YOLO(object):
    _defaults = {
        #--------------------------------------------------------------------------#
        ...
        #
        # After training, there are multiple weight files in the logs folder. Just choose the one with the lower loss on the verification set.
        # A lower loss on the validation set does not mean a higher mAP, it only means that the weight has better generalization performance on the validation set.
    ...
    def __init__(self, **kwargs):
        ...
        #---------------------------------------------------#
        # Get the number of categories and a priori boxes
        #---------------------------------------------------#
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        #---------------------------------------------------------#
        # Whether to perform target cropping
        #---------------------------------------------------------#
        if crop:
        ...
        for i, c in list(enumerate(top_label)):
            ...
            # Create large fonts (10x size)
            large_font = ImageFont.truetype(font='model_data/simhei.ttf', size=np.floor(3e-1 * image.size[1] + 0.5).astype('int32'))
            ...
            # Draw category letters in the upper right corner (white font, converted to uppercase)
            class_label = predicted_class.upper()
            class_label_size = draw.textbbox((0, 0), class_label, font=large_font)[2:]
            # At the upper right corner, leave some margins
            class_text_origin = np.array([image.size[0] - class_label_size[0] - 20, 20])
            draw.text(class_text_origin, class_label, fill=(255, 255, 255), font=large_font)
            
            # Draw confidence above bounding box
            score_label = '{:.2f}'.format(score)
            score_label_size = draw.textbbox((0, 0), score_label, font=font)[2:]
            if top - score_label_size[1] >= 0:
                score_text_origin = np.array([left, top - score_label_size[1]])
            else:
                score_text_origin = np.array([left, top + 1])
            draw.rectangle([tuple(score_text_origin), tuple(score_text_origin + score_label_size)], fill=self.colors[c])
            draw.text(score_text_origin, score_label, fill=(0, 0, 0), font=font)
            
            print(class_label, score, top, left, bottom, right)
            del draw
