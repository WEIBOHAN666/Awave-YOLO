    ...
    if true_p_bar != "":
        ...
        for i, val in enumerate(sorted_values):
            ...
            if i == (len(sorted_values)-1): # largest bar
                adjust_axes(r, t, fig, axes)
    # set window title
    # fig.canvas.set_window_title(window_title) # Comment out this line to avoid matplotlib compatibility issues
...
def get_map(MINOVERLAP, draw_plot, score_threhold=0.5, path = './map_out'):
    ...
    with open(RESULTS_FILES_PATH + "/results.txt", 'w') as results_file:
        ...
        if n_classes == 0:
            print("No category was detected, please check whether the label information and classes_path in get_map.py have been modified.")
            return 0
        results_file.write("\n# mAP of all classes\n")
...
def preprocess_gt(gt_path, class_names):
    ...
    for i, image_id in enumerate(image_ids):
        ...
        image['height']    = 1
        #-----------------------------------------------------------------#
        # Thanks for the reminder to learn English more
        # Fixed 'Results do not correspond to current coco set' issue
        #-----------------------------------------------------------------#
