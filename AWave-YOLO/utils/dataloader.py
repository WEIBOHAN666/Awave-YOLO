...
class YoloDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # No random enhancement of data is performed during verification
        #---------------------------------------------------#
        ...
        #---------------------------------------------------#
        # Preprocess the real box
        #---------------------------------------------------#
        nL          = len(box)
        labels_out  = np.zeros((nL, 6))
        if nL:
            #---------------------------------------------------#
            # Normalize the real frame and adjust it to between 0-1
            #---------------------------------------------------#
            box[:, [0, 2]] = box[:, [0, 2]] / self.input_shape[1]
            box[:, [1, 3]] = box[:, [1, 3]] / self.input_shape[0]
            #---------------------------------------------------#
            # The parts numbered 0 and 1 are the center of the real frame
            # The parts numbered 2 and 3 are the width and height of the real frame.
            # The part numbered 4 is the type of real frame
            #---------------------------------------------------#
            box[:, 2:4] = box[:, 2:4] - box[:, 0:2]
            box[:, 0:2] = box[:, 0:2] + box[:, 2:4] / 2
            
            #---------------------------------------------------#
            # Adjust the order to fit the training format
            # The part with serial number 0 in labels_out is processed during collect
            #---------------------------------------------------#
            labels_out[:, 1] = box[:, -1]
            labels_out[:, 2:] = box[:, :4]
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.3, hue=.1, sat=0.7, val=0.4, random=True):
        ...
        import re
        # Match path part (ending with .jpg or .png)
        path_match = re.match(r'^(.+\.(?:jpg|jpeg|png|bmp|gif))\s*(.*)$', annotation_line)
        if path_match:
            image_path = path_match.group(1)
            box_str = path_match.group(2)
            boxes = box_str.split() if box_str else []
        else:
        ...
        if not random:
            ...
            #---------------------------------#
            # Add gray bars to the redundant parts of the image
            #---------------------------------#
    ...
    def get_random_data_with_Mosaic(self, annotation_line, input_shape, jitter=0.3, hue=.1, sat=0.7, val=0.4):
        ...
        for line in annotation_line:
            #---------------------------------#
            # Split each line and handle paths containing spaces.
            #---------------------------------#
            import re
            # Match path part (ending with .jpg or .png)
            path_match = re.match(r'^(.+\.(?:jpg|jpeg|png|bmp|gif))\s*(.*)$', line)
            if path_match:
                image_path = path_match.group(1)
                box_str = path_match.group(2)
                boxes = box_str.split() if box_str else []
            ...
            #-----------------------------------------------#
            # Place the pictures corresponding to the positions of the four divided pictures.
            #-----------------------------------------------#
            if index == 0:
        ...
        #---------------------------------#
        # Split pictures and put them together
        #---------------------------------#
