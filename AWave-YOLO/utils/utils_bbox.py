...
def check_version(current: str = "0.0.0",
    ...
    def parse_version(version_str):
        # Handle extra information in the version string, such as '0+cpu'
        ...
        for part in parts:
            ...
            if num:
                clean_parts.append(int(num))
            else:
                clean_parts.append(0)
        # Make sure there are at least 3 sections
...
# Directly set TORCH_1_10 to True, because our torch version should meet the requirements
TORCH_1_10 = True
...
class DecodeBox():
    ...
    def yolo_correct_boxes(self, box_xy, box_wh, input_shape, image_shape, letterbox_image):
        #-----------------------------------------------------------------#
        # The y-axis is placed in front because it is convenient to multiply the prediction box and the width and height of the image.
        #-----------------------------------------------------------------#
        box_yx = box_xy[..., ::-1]
        box_hw = box_wh[..., ::-1]
    ...
    def non_max_suppression(self, prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5, nms_thres=0.4):
        #----------------------------------------------------------#
        # Convert the format of the prediction results to the format in the upper left corner and lower right corner.
        ...
        for i, image_pred in enumerate(prediction):
            #----------------------------------------------------------#
            # Take max for the category prediction part.
            ...
            #----------------------------------------------------------#
            # Leveraging Confidence for First Round Screening
            #----------------------------------------------------------#
            ...
            # The content of 6 is: x1, y1, x2, y2, class_conf, class_pred
            #-------------------------------------------------------------------------#
            detections = torch.cat((image_pred[:, :4], class_conf.float(), class_pred.float()), 1)

            #------------------------------------------#
            # Get all categories included in the prediction results
            #------------------------------------------#
            ...
            for c in unique_labels:
                #------------------------------------------#
                # Obtain all prediction results after filtering by a certain category of scores
                #------------------------------------------#
                detections_class = detections[detections[:, -1] == c]
                #------------------------------------------#
                # Using the official non-maximum suppression will be faster!
                # Filter out the boxes with the highest score of the same category in a certain area
                #------------------------------------------#
                keep = nms(
                    detections_class[:, :4],
                    detections_class[:, 4],
                ...
                # # Take out the category with the highest confidence and judge step by step to determine whether the degree of overlap is greater than nms_thres. If so, remove it.
...
if __name__ == "__main__":
    ...
    def get_anchors_and_decode(input, input_shape, anchors, anchors_mask, num_classes):
        ...
        #-----------------------------------------------#
        # When the input is 640x640, input_shape = [640, 640] input_height = 20, input_width = 20
        #   640 / 20 = 32
        ...
        #-----------------------------------------------#
        # Get confidence whether there is an object 0 - 1
        #-----------------------------------------------#
        ...
        #----------------------------------------------------------#
        # Generate a grid, the center of the a priori box, the upper left corner of the grid
        #   batch_size,3,20,20
        #   range(20)
        #   [
        #       [0, 1, 2, 3 ..., 19], 
