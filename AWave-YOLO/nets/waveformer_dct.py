...
class Wave2D(nn.Module):
    ...
    def __init__(self, infer_mode=False, res=14, dim=96, hidden_dim=96, **kwargs):
        ...
        # Damping-wave velocity modulation (SWM) of semantic perception
        # Predict parameters using global pooling and low-dimensional MLP
        self.global_pool = nn.AdaptiveAvgPool2d(1)
        self.param_mlp = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim // 2),
            nn.GELU(),
            nn.Linear(hidden_dim // 2, 2)  # Output c and alpha
        )
        # Initialization parameter range
        self.c_bias = nn.Parameter(torch.ones(1) * 1)
        self.alpha_bias = nn.Parameter(torch.ones(1) * 0.1)
        
        # Direction-aware weight parameters - use the formula w(θ) = 1 + Σ_{k=1}^K a_k cos(kθ + φ_k)
        self.K = 2  # In engineering, K=2 or 3 is enough
        self.a = nn.Parameter(torch.zeros(self.K), requires_grad=True)  # Amplitude parameters
        self.phi = nn.Parameter(torch.zeros(self.K), requires_grad=True)  # Phase parameter
        self.direction_weight = nn.Parameter(torch.ones(1) * 0.5)  # direction weight scaling factor
    ...
    def get_direction_map(self, resolution=(224, 224), device=torch.device("cpu"), dtype=torch.float):
        """Generate direction weight map - use formula w(θ) = 1 + Σ_{k=1}^K a_k cos(kθ + φ_k)"""
        resh, resw = resolution
        
        # Calculate the coordinates of each frequency component
        freq_n = torch.linspace(-resh//2, resh//2, resh, device=device, dtype=dtype).view(-1, 1)
        freq_m = torch.linspace(-resw//2, resw//2, resw, device=device, dtype=dtype).view(1, -1)
        
        # Calculate the angle (relative to the center) of each point in the range [-π, π]
        angle = torch.atan2(freq_n, freq_m)
        
        # Calculate the direction weight w(θ) = 1 + Σ_{k=1}^K a_k cos(kθ + φ_k)
        weight = torch.ones((resh, resw), device=device, dtype=dtype)
        
        # Get the current amplitude and phase parameters
        a = self.a.to(device)
        ...
        for k in range(1, self.K + 1):
            if k <= len(a):
                weight += a[k-1] * torch.cos(k * angle + phi[k-1])
    ...
    def forward(self, x: torch.Tensor, freq_embed=None, test_index=None):
        ...
        params = self.param_mlp(global_feat)
        # Make sure the parameters are positive
        c = F.softplus(params[:, 0:1]) + self.c_bias
        alpha = F.softplus(params[:, 1:2]) + self.alpha_bias
        ...
        # Apply direction-aware weights - use the formula w(θ) = 1 + Σ_{k=1}^K a_k cos(kθ + φ_k)
        direction_weight_map = self.get_direction_map((H, W), device=x.device)
        
        # Apply directional weights to frequency domain features
        direction_weight_map = direction_weight_map.view(1, H, W, 1)  # Adjust the shape to (1, H, W, 1) to broadcast in the channel dimension
        x_u0 = x_u0 * (1 + self.direction_weight * direction_weight_map)
        x_v0 = x_v0 * (1 + self.direction_weight * direction_weight_map)
        
        if freq_embed is None:
            freq_embed = torch.zeros(B, H, W, self.hidden_dim, device=x.device, dtype=x.dtype)
        t = self.to_k(freq_embed)
        c_t = c.view(B, 1, 1, 1) * t
        cos_term = torch.cos(c_t)
        eps = 1e-8
        sin_term = torch.sin(c_t) / (c.view(B, 1, 1, 1) + eps)
        wave_term = cos_term * x_u0
        velocity_term = sin_term * (x_v0 + (alpha.view(B, 1, 1, 1) / 2) * x_u0)
