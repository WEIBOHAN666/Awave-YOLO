...
def autopad(k, p=None, d=1):  
    ...
    # Automatic padding is performed on the input feature layer according to the Same principle.
    if d > 1:
        # actual kernel-size
...
class C2f(nn.Module):
    ...
    def forward(self, x):
        ...
        # The residual structure is retained each time, and then stacked together to densely
        y.extend(m(y[-1]) for m in self.m)
        return self.cv2(torch.cat(y, 1))
    
class SPPF(nn.Module):
    # SPP structure, max pooling of 5, 9, and 13 max pooling kernels.
...
class Backbone(nn.Module):
    ...
    def forward(self, x):
        ...
        feat2 = x
        #-----------------------------------------------#
        # The output of dark5 is 1024 * deep_mul, 20, 20, which is an effective feature layer
        #-----------------------------------------------#
        x = self.dark5(x)
        feat3 = x
